import Image from 'next/image'

interface Props {
  children: React.ReactNode
  className?: string
}

export const InfoText = ({ children, className }: Props) => {
  return (
    <div className={`${className} flex gap-2 rounded-xl border-2 border-blue/20 bg-blue/10 p-3`}>
      <Image
        alt="Инфо"
        src="data:image/svg+xml;base64,"
        width={20}
        height={20}
      ></Image>
      <span>{children}</span>
    </div>
  )
}

export const WarningText = ({ children, className }: Props) => {
  return (
    <div className={`${className} flex gap-2 rounded-xl border-2 border-mango/20 bg-mango/10 p-3`}>
      <Image
        alt="Инфо"
        src="data:image/svg+xml;base64,"
        width={20}
        height={20}
      ></Image>
      <span>{children}</span>
    </div>
  )
}

export const DangerText = ({ children, className }: Props) => {
  return (
    <div
      className={`${className} flex gap-2 rounded-xl border-2 border-danger/20 bg-danger/10 p-3`}
    >
      <Image
        alt="Инфо"
        src="data:image/svg+xml;base64,"
        width={20}
        height={20}
      ></Image>
      <span>{children}</span>
    </div>
  )
}
